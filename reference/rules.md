...
このファイルは、源流である k16shikano「日本語技術文書の文章規範」の全規範を、**要約せず原文のまま**収録したものである。各観点の悪い例・良い例も省略しない。

- 源流：k16shikano「日本語技術文書の文章規範」（Unlicense / パブリックドメイン）
  <https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d>

源流は単一ファイルに全規範を inline していた。本リポジトリでは、運用のために [SKILL.md](../SKILL.md)（モード運用）・[checklist.md](checklist.md)（点検と採点）・[ng-vocab.md](ng-vocab.md)（AI 臭語彙）に役割を分けたが、**規範本文そのものはこの rules.md が源流の完全な写し**であり、判断に迷ったときはここに戻る。
...
規範どうしが衝突したら、次の順で優先する。

1. **論証の厳密さ** — 正確さと誠実さ。必要な不確実性は残す。簡潔さや形式より優先する。
2. **読み手の負荷の管理・冗長の排除** — 分かりやすさと簡潔さ。
3. その他の形式・演出に関する観点。
